...
const menuItems = [
  { icon: (
    ...
    ), label: 'Documentation', shortcut: 'D', href: '#' },
  { icon: (
    ...
    ), label: 'Keyboard shortcuts', shortcut: 'S', href: '#' },
  { icon: (
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="absolute bottom-10 right-0 z-50 min-w-[240px] bg-neutral-900 border border-zinc-800 rounded-xl shadow-xl py-2 px-1 text-sm"
          style={{ boxShadow: '0 8px 32px 0 rgba(0,0,0,0.25)' }}
        >
          {menuItems.map((item, idx) =>
            item === 'divider' ? (
              <div key={idx} className="my-2 border-t border-zinc-800" />
            ) : (
              <a
                key={item.label}
                href={item.href}
                className="flex items-center gap-3 px-3 py-2 rounded-lg text-zinc-200 hover:bg-zinc-800 transition-colors group"
                tabIndex={0}
              >
                <span className="text-lg opacity-80">{item.icon}</span>
                <span className="flex-1">{item.label}</span>
                {item.shortcut && (
                  <kbd className="px-2 py-1 rounded bg-zinc-800 border border-zinc-700 text-xs font-mono text-zinc-300 ml-2">
                    {item.shortcut}
                  </kbd>
                )}
              </a>
