  ...
  return (
    <div className={`flex flex-col flex-1 min-h-0 bg-bg text-text ${themeClass}`}>
      ...
      <div className="flex items-center gap-2 mb-2 bg-bg">
        <span className="text-gray-400">{t('realtime.message')}</span>
        <select className="bg-transparent text-text font-semibold px-1 py-0 focus:outline-none">
          <option>{t('realtime.format_options.json')}</option>
          <option>{t('realtime.format_options.raw')}</option>
        </select>
        <button className="text-blue-500 flex items-center gap-1 font-semibold ml-2">
          <span className="material-icons" style={{ fontSize: 20 }}>send</span> {t('realtime.actions.send')}
        </button>
        <label className="flex items-center gap-1 text-xs text-gray-400 cursor-pointer ml-2">
          <input type="checkbox" className="accent-blue-600" /> {t('realtime.actions.clear_input')}
        </label>
        <button className="ml-2 text-gray-400 hover:text-white" title={t('common.help')}>
          <span className="material-icons">help_outline</span>
        </button>
        <button className="text-gray-400 hover:text-white" title={t('common.delete')}>
          <span className="material-icons">delete</span>
        </button>
        <button className="text-gray-400 hover:text-white" title={t('common.history')}>
          <span className="material-icons">history</span>
        </button>
        <button className="text-gray-400 hover:text-white" title={t('common.format')}>
          <span className="material-icons">format_align_left</span>
        </button>
        <button className="text-gray-400 hover:text-white" title={t('common.copy')}>
